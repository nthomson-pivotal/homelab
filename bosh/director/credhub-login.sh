...
credhub_admin_secret=$(bosh int "$DIR/state/creds.yml" --path="/credhub_admin_client_secret")
...
bosh int "$DIR/state/creds.yml" --path="/uaa_ssl/ca" > "${tmp_uaa_ca_file}"
bosh int "$DIR/state/creds.yml" --path="/credhub_tls/ca" > "${tmp_credhub_ca_file}"
